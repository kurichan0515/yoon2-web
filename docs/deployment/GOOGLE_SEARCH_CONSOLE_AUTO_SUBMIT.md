...
2. **サービスアカウントJSONを追加**
   ...
   - **Value**: ダウンロードしたJSONファイルの内容全体をコピー＆ペースト
     ...
     - 改行やスペースも含めて完全にコピー
     - **重要**: JSON全体を1つの値として設定してください（改行を含む）
     - または、base64エンコードした値を設定することも可能です
...
### エラー: "Invalid JSON" または "Unexpected token"

**原因**: GitHub SecretsのJSONが正しくコピーされていない、または文字化けしている

**解決方法**:
1. JSONファイルを再度開いて、すべての内容をコピー
2. GitHub Secretsを削除して再作成
3. 改行やスペースも含めて完全にコピーすることを確認
4. **重要**: JSON全体を1つの値として設定してください
   - 複数行のJSONをそのまま貼り付けることができます
   - GitHub Secretsは複数行の値をサポートしています
5. それでもエラーが出る場合:
   - JSONファイルを1行に変換して設定する
   - または、base64エンコードして設定する
   ```bash
   # base64エンコード（オプション）
